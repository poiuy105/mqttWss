...
class CloudTTSPlayer private constructor() {
    ...
    companion object {
        ...
        const val API_CHINESE_TTS_FLITE = 4  // ChineseTtsTflite（完全离线中文TTS）🆕
    ...
    /** ChineseTtsTflite引擎实例（用于离线中文TTS） */
    private var chineseTtsEngine: ChineseTtsEngine? = null
    ...
    // ========== 可配置参数（从Setting页面设置） ==========
    var currentApiIndex: Int = API_CHINESE_TTS_FLITE  // 默认使用ChineseTtsTflite离线中文TTS
    ...
    fun speak(text: String, force: Boolean = false) {
        ...
        when (currentApiIndex) {
            ...
            API_CHINESE_TTS_FLITE -> speakWithChineseTtsFlite(text)
